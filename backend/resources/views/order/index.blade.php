                ...
                <tbody>
                    @foreach ($orders as $order)
                        <tr class="border-b">
                            ...
                            <td>
                                <span
                                    class="text-white p-2 rounded {{ $order->status === OrderStatus::Paid->value ? 'bg-emerald-500' : 'bg-gray-500' }}  ">{{ $order->status }}</span>
                            </td>
                            <td>${{ $order->total_price }}</td>
                            <td class="whitespace-nowrap">${{ $order->items()->count() }} item(s)</td>
                            <td class="flex gap-3">
                                @if (!$order->isPaid())
