            ...
            <table class="table-auto w-full">
                <thead>
                    <tr>
                        ...
                        >
                        <TableHeaderCell
                            v-if="authUser.role === 'gestionnaire'"
                            class="border-b-2 p-2 text-left"
                            field="retail_store_name"
                            :sort-field="sortField"
                            :sort-direction="sortDirection"
                            @click="sortUsers"
...
const retailStoreIdSelected = ref(null);
...
onMounted(async () => {
    try {
        ...
        await Promise.all([
            usersStore.fetchAll(),
            retailStoresStore.fetchAll("Tous les points de vente"),
...
async function handleSelectChange() {
    await usersStore.fetchAll({
        retailStoreId: retailStoreIdSelected.value,
    });
}
