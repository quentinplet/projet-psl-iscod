  ...
  <div class="bg-white p-4 rounded-lg shadow animate-fade-in-down">
    ...
    <div>
      <table class="table-auto w-full">
        ...
        <tbody v-else>
          <tr v-for="(order, index) in orders.data.data" :key="order.id">
            ...
            <td class="border-b p-2 w-60">
              ...
              <span
                ...
                :class="[
                  colorOrderStatus.find((status) => status.key === order.status)
...
onMounted(async () => {
  await Promise.all([
    ...
    userRole.value !== "acheteur" &&
      retailStoresStore.fetchSelectOptRetailStores("Tous les points de vente"),
    fetchAllOrders(),
  ]);
});
